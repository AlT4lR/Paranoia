...
def setup_gui(master: tk.Tk, image_path: str):
    """Initializes the visual interface for Hu Tao's terminal."""
    ...
    # --- Header Frame (Avatar, Name, Emotion) ---
    info_frame = tk.Frame(root, bg=config.HU_TAO_DARK)
    info_frame.pack(side=tk.TOP, fill=tk.X)

    # Avatar Image with Masking
    try:
        ...
        image_label.image = photo # Persistent reference to prevent garbage collection
        image_label.pack(side=tk.LEFT, padx=15, pady=10)
    except Exception:
        # Fallback to a red square if image processing fails
        image_label = tk.Label(info_frame, width=10, height=5, bg=config.HU_TAO_RED)
        image_label.pack(side=tk.LEFT, padx=15, pady=10)

    # Branding
    text_frame = tk.Frame(info_frame, bg=config.HU_TAO_DARK)
    text_frame.pack(side=tk.LEFT)
    tk.Label(text_frame, text="Hu Tao", font=("Baskerville", 22, "bold"), 
             fg=config.HU_TAO_RED, bg=config.HU_TAO_DARK).pack(anchor="w")
    tk.Label(text_frame, text=f"v1.5.3Resilience", font=("Arial", 10), 
             fg="gray", bg=config.HU_TAO_DARK).pack(anchor="w")

    # Dynamic Emotion Indicator
    emotion_label = tk.Label(info_frame, text="Neutral", font=("Arial", 12), 
                             fg=config.HU_TAO_MESSAGE, bg=config.HU_TAO_DARK)
    ...
    chat_log = tk.Text(chat_frame, bg="#1e1e1e", fg="white", font=("Courier New", 11), 
                       state=tk.DISABLED, wrap=tk.WORD, relief=tk.FLAT)
    chat_log.pack(fill=tk.BOTH, expand=True)
    # Define tags for colorful dialogue
    chat_log.tag_config("hutao", foreground=config.HU_TAO_MESSAGE)
    chat_log.tag_config("user", foreground="white")

    # --- Input Bar ---
    input_frame = tk.Frame(root, bg=config.HU_TAO_DARK)
    input_frame.pack(fill=tk.X, side=tk.BOTTOM)
    
    message_entry = tk.Entry(input_frame, bg="#2d2d2d", fg="white", 
                             font=("Courier New", 12), insertbackground="white", relief=tk.FLAT)
    message_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=10, pady=10)
    
    send_button = tk.Button(input_frame, text="Send", bg=config.HU_TAO_RED, 
                            fg="white", width=10, activebackground="#a12020", relief=tk.FLAT)
    send_button.pack(side=tk.RIGHT, padx=10)

def update_avatar(emotion):
    """Swaps the character portrait and updates the mood indicator."""
    global image_label, emotion_label
    if image_label is None or not image_label.winfo_exists(): return

    # Map emotions to asset files
    img_map = {
        "happy": "happy.jpg",
    ...
    # Emotional color coding for the text label
    color_map = {
        "happy": "#FFD700",  # Gold
        "sad": "#87CEEB",    # Sky Blue
        "mischief": "#FF4500", # OrangeRed
        "surprised": "#ADFF2F" # GreenYellow
    }
    ...
    try:
        ...
        emotion_label.config(
            ...
            fg=color_map.get(emotion, config.HU_TAO_MESSAGE)
        )
    except Exception:
        pass

def update_chat_log(message, sender="user"):
    """Inserts text into the scrolling chat history."""
    global chat_log
    if chat_log is None or not chat_log.winfo_exists(): return 
